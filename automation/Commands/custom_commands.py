...
from .browser_commands import get_website, bot_mitigation, screenshot_full_page, dump_page_source
    ...
    for i in range(num_links):
        ...
        for link in links:
            try:
                ...
                logger.error('Error while looping through links...: %s' % str(e))
...
def _find_and_fill_form(webdriver, user_data, visit_id, debug, browser_params, manager_params, logger):
    ...
    debug_form_pre_initial = 'form_initial_presubmit'
    debug_form_post_initial = 'form_initial_result'
    debug_form_pre_followup = 'form_followup_presubmit'
    debug_form_post_followup = 'form_followup_result'
    debug_page_source_initial = 'src_initial'
    debug_page_source_followup = 'src_followup'
            ...
            if window != main_handle:
                ...
                if follow_up_form is None:
                    ...
                    if follow_up_form is not None:
                        if debug:
                            dump_page_source(visit_id, webdriver, manager_params, debug_page_source_followup)
                            logger.debug('Found a newsletter form in another window')
                        _form_fill_and_submit(follow_up_form, user_info, webdriver, visit_id, True, browser_params, manager_params, debug_form_pre_followup if debug else None)
...
def _form_fill_and_submit(form, user_info, webdriver, visit_id, clear, browser_params, manager_params, screenshot_suffix):
